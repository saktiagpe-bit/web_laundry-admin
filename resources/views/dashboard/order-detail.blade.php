...
@push('scripts')
<!-- Load Supabase JS SDK dari CDN untuk fitur Realtime Tracking di Dashboard -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (supabaseUrl && supabaseKey) {
            try {
                ...
                // Dengarkan pembaruan status secara langsung (Realtime WebSocket) dari tabel order_statuses di Supabase
                const channel = supabaseClient
                    ...
                    .on('postgres_changes', {
                        ...
                        // Reload halaman agar tampilan admin terupdate otomatis
        ...
        // halaman otomatis ngecek status lewat HTTP Polling (Ajax request) setiap 4 detik ke server Laravel
        setInterval(function() {
            if (!usingRealtime) {
                fetch(`/orders/${orderId}/status`)
                    .then(response => response.json())
                    .then(data => {
                        // Jika status di database Supabase berubah (dicek via Laravel), reload halaman
